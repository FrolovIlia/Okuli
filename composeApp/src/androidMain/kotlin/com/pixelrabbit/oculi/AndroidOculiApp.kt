...
@Composable
fun AndroidOculiApp(appOpenAdManager: AppOpenAdManager) {
    ...
    if (showSplash) {
        ...
        Column(
            ...
            Button(
                onClick = {
                    android.util.Log.d("OculiDebug", "Тестовая кнопка нажата")
                    if (appOpenAdManager.isAdAvailable()) {
                        appOpenAdManager.showIfAvailable {
                            android.util.Log.d("OculiDebug", "Колбэк после рекламы (кнопка)")
                        }
                    }
                },
        ...
        if (pendingAdAvailable) {
            LaunchedEffect(Unit) {
                delay(100L)
                appOpenAdManager.showIfAvailable {
                    android.util.Log.d("OculiDebug", "Реклама показана после Splash")
                }
